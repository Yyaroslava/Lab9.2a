    ...
    while (true) {
        ...
        if (action == 4) {
            ...
            if (found == -1) {
                ...
                cout << "   спеціальність: " << p[found].spesialnist << endl;
                cout << "   оцінка з фізики: " << p[found].mark_phys << endl;
                cout << "   оцінка з математики: " << p[found].mark_math << endl;
                cout << "   оцінка з інформатики: " << p[found].mark_inf << endl;
            }
        }
    }
...
void Calculate_Average_Mark(Student* p, const int N) {
    for (int i = 0; i < N; i++) {
        p[i].average = round((p[i].mark_phys + p[i].mark_math + p[i].mark_inf) / 3.*100)/100.;
    }
}
